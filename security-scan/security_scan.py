...
def generateCVEStats(relevant_cves, cve_info, filename):
	...
	with open(filename, 'w') as htmlFile:
		...
		htmlFile.write('   <td align="center"><h3>%s</h3></td><td align="center"><h3>%s</h3></td><td align="center"><h3>%s</h3></td><td align="center><h3>%s</h3></td></td>' % (severity_stats['critical'], severity_stats['high'], severity_stats['medium'], severity_stats['low'], severity_stats['negligible']))
		htmlFile.write('</tbody></table></td></tr>')
		htmlFile.write('<tr><td colspan="6" style="background-color: #111;" width="100%"><h3><center>Last updated on ' + datetime.now().strftime("%Y-%m-%d %H:%M") + '</center></h3></td></tr></table>' )
